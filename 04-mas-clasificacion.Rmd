...
que la tiene. Estas consecuencias dependen de cómo son son los tratamientos consecuentes, y e qué tan peligrosa es la enfermedad.
...
$C_{i,j} =$ Número de casos de la clase verdadera j que son clasificados como clase i
 por el clasificador
 ```
...
library(dplyr)
tabla <- data_frame('pred\obs' = c('positivo.pred','negativo.pred','total'),
...
La razón es que para cualquier punto de corte (con sensibilidad mayor a 0.4) en el clasificador de una variable, existe otro clasificador en la curva azul (todas las variable), que domina al primero. La excepción es para clasificadores de valores de sensibilidad baja, con tasas de falsos positivos muy chicas: 
